...
def product_match(row):
    ...
    for t in range(len(right_type)):
        if left_number[0] == right_number[t] and (left_type[0] in right_type[t] or right_type[t] in left_type[0]):
            return 1
    for t in range(len(left_type)):
        if left_number[t] == right_number[0] and (left_type[t] in right_type[0] or right_type[0] in left_type[t]):
            return 1
...
def compute_matching(pairs_df, dataset_df):
    ...
    matching_pairs_df = pairs_df[pairs_df['predict'] == 1][['left_spec_id', 'right_spec_id', 'left_spec_title', 'right_spec_title']]
